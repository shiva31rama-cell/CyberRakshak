...
const fallbackReply = (message) => {
  ...
  if (text.includes("phish")) {
    return "⚠️ Phishing is a deceptive message or site designed to steal information. Verify the sender and domain independently, avoid unexpected links or attachments, and never enter passwords or OTPs from a suspicious message.";
  }
  if (text.includes("otp")) {
    return "🔑 Never share an OTP with anyone. Banks and legitimate support teams do not need your OTP to receive or release a payment.";
  }
  if (text.includes("upi")) {
    return "💳 For UPI safety, verify the recipient and amount before approving a transaction. Your UPI PIN is only for authorizing payments—never share it or enter it to receive money.";
  }
  if (text.includes("password")) {
    return "🔐 Use a unique, long password for every important account and enable multi-factor authentication where available. A password manager can help you avoid reuse.";
  }
  if (text.includes("scam")) {
    return "🚨 Stop communicating with the suspected scammer, preserve relevant evidence, contact your bank/service immediately if money or an account is involved, and use CyberRakshak's scam-reporting flow to document the incident.";
  }
...
exports.chat = async (req, res) => {
  ...
  try {
    ...
    if (!apiKey || !apiUrl || !model || apiKey === "replace-with-your-provider-key") {
      return res.json({
        success: true,
        reply: fallbackReply(latestUserMessage),
        provider: "CyberRakshak safety fallback",
      });
    }
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: process.env.AI_SYSTEM_PROMPT?.trim() || DEFAULT_SYSTEM_PROMPT,
            },
            ...safeMessages,
          ],
        }),
