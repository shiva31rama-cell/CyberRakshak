...
const starterMessage = { id: "welcome", text: "Namaste 👋 I’m CyberRakshak AI. I can explain a suspicious situation, teach you a safety skill, help you respond after an incident, or guide you to the right CyberRakshak feature.", sender: "bot", timestamp: new Date().toISOString() };
...
function formatReply(text) {
  ...
  return lines.map((line, index) => <span key={`${index}-${line}`}>{line}{index < lines.length - 1 ? <br /> : null}</span>);
}

function CyberToolCard({ tool, onClick }) {
  return <button type="button" className="cyber-tool-card" onClick={onClick}><span className="cyber-tool-icon" aria-hidden="true">{tool.icon}</span><span><strong>{tool.label}</strong><small>Open guided mode</small></span><span className="cyber-tool-arrow" aria-hidden="true">→</span></button>;
}
...
function Chatbot() {
  ...
  const persist = (nextMessages) => {
    ...
    setMessages(trimmed);
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(trimmed)); } catch { /* Storage is optional. */ }
  ...
  const sendMessage = async (text, toolName = "chat") => {
    ...
    setIsLoading(true);
    try {
      const response = await sendChatMessage({ messages: nextMessages.map((message) => ({ role: message.sender === "user" ? "user" : "assistant", content: message.text })), language, ageGroup, previousInteractionId: interactionId });
  ...
  const handleSend = (event) => { event.preventDefault(); sendMessage(inputValue); };
  const handleTool = (tool) => {
    if (tool.label === "Check message") { navigate("/check"); setIsOpen(false); return; }
    if (tool.label === "I need help") { navigate("/emergency-help"); setIsOpen(false); return; }
  ...
  const copyMessage = async (text) => {
    try { await navigator.clipboard.writeText(text); } catch { setError("Copy is unavailable in this browser context."); }
  };
    ...
    <div className={`chatbot-container ${isOpen ? "open" : ""}`}>
      {isOpen ? (
        <div className="chatbot-window" role="dialog" aria-label="CyberRakshak AI Safety Copilot" aria-modal="true">
          <header className="chatbot-header"><div className="chatbot-brand"><div className="chatbot-avatar" aria-hidden="true">🛡️</div><div><strong>CyberRakshak AI</strong><span><i className="status-dot" /> Safety Copilot • {language}</span></div></div><div className="chatbot-header-actions"><button type="button" onClick={clearConversation} aria-label="Start a new conversation">＋</button><button type="button" onClick={toggleOpen} aria-label="Close AI assistant">×</button></div></header>
          <div className="chatbot-context-bar"><span>Advanced guidance with a safe fallback</span><div><select value={language} aria-label="AI language" onChange={(event) => changeLanguage(event.target.value)}>{languages.map((item) => <option key={item}>{item}</option>)}</select><select value={ageGroup} aria-label="AI age group" onChange={(event) => saveAge(event.target.value)}><option value="">Profile</option><option value="teen">Teen</option><option value="young-adult">Young Adult</option><option value="adult">Adult</option><option value="senior">Senior</option></select></div></div>
          <div className="chatbot-tool-switcher" role="tablist" aria-label="CyberRakshak AI modes"><button type="button" className={activeTool === "chat" ? "active" : ""} onClick={() => setActiveTool("chat")}>💬 Chat</button><button type="button" className={activeTool === "teach" ? "active" : ""} onClick={() => setActiveTool("teach")}>🎓 Learn</button><button type="button" className={activeTool === "privacy" ? "active" : ""} onClick={() => setActiveTool("privacy")}>🔐 Safety</button></div>
          <div className="chatbot-messages" aria-live="polite">
            {messages.map((message, index) => <article key={message.id} className={`message ${message.sender === "user" ? "user" : "bot"}`}><div className="message-meta"><span>{message.sender === "user" ? "You" : "CyberRakshak AI"}</span><time dateTime={message.timestamp}>{new Date(message.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time></div><div className="message-content">{formatReply(message.text)}</div>{message.sender === "bot" && index > 0 ? <div className="message-actions"><button type="button" onClick={() => copyMessage(message.text)}>Copy</button><button type="button" onClick={handleRegenerate}>Regenerate</button></div> : null}{message.provider === "deterministic-fallback" ? <small className="fallback-note">Local safety fallback used</small> : null}</article>)}
          ...
          <form onSubmit={handleSend} className="chatbot-input-form"><textarea value={inputValue} onChange={(event) => setInputValue(event.target.value)} placeholder={location.pathname === "/check" ? "Ask about this suspicious message…" : "Message CyberRakshak AI…"} maxLength={2000} rows={1} disabled={isLoading} aria-label="Message CyberRakshak AI"/><button type="submit" disabled={isLoading || !inputValue.trim()} aria-label="Send message">↑</button></form>
