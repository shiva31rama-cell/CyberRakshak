...
function loadProfile() {
  try {
    return JSON.parse(localStorage.getItem(PROFILE_KEY)) || { ageGroup: "", language: "English" };
  } catch {
...
function formatReply(text) {
  return String(text || "").split("\n").map((line, index) => (
    <span key={`${index}-${line}`}>{line}{index < String(text || "").split("\n").length - 1 ? <br /> : null}</span>
  ));
}
...
function Chatbot() {
  ...
  const sendMessage = async (text, toolName = "chat") => {
    ...
    try {
      ...
      const botMessage = {
        id: makeId(),
        text: response.reply || "I could not generate a useful response right now.",
        sender: "bot",
        timestamp: new Date().toISOString(),
  ...
  return (
    <div className={`chatbot-container ${isOpen ? "open" : ""}`}>
      {isOpen && (
        <div className="chatbot-window" role="dialog" aria-label="CyberRakshak AI Safety Copilot">
          <header className="chatbot-header">
            <div className="chatbot-brand">
              <div className="chatbot-avatar" aria-hidden="true">🛡️</div>
              <div>
                <strong>CyberRakshak AI</strong>
                <span><i className="status-dot" /> Safety Copilot • {profile.language}</span>
              </div>
            </div>
          ...
          <div className="chatbot-context-bar">
            ...
            <div>
              <select value={profile.language} aria-label="AI language" onChange={(event) => saveProfile({ ...profile, language: event.target.value })}>
                <option>English</option>
                <option>Telugu</option>
                <option>Hindi</option>
              </select>
              <select value={profile.ageGroup} aria-label="AI age group" onChange={(event) => saveProfile({ ...profile, ageGroup: event.target.value })}>
            ...
            {messages.map((message, index) => (
              <article key={message.id} className={`message ${message.sender === "user" ? "user" : "bot"}`}>
                <div className="message-meta">
                  <span>{message.sender === "user" ? "You" : "CyberRakshak AI"}</span>
                ...
                {message.sender === "bot" && index > 0 && (
                  <div className="message-actions">
                    <button type="button" onClick={() => copyMessage(message.text)}>Copy</button>
                    <button type="button" onClick={handleRegenerate}>Regenerate</button>
                  </div>
                )}
                {message.provider === "deterministic-fallback" && <small className="fallback-note">Local safety fallback used</small>}
              </article>
            ))}
            {isLoading && (
              <article className="message bot">
                <div className="message-meta"><span>CyberRakshak AI</span><span>thinking…</span></div>
                <div className="message-content"><span className="thinking"><span /><span /><span /></span></div>
              </article>
            )}
            {error && <div className="chatbot-error" role="alert">⚠️ {error}</div>}
            <div ref={bottomRef} />
          </div>

          {messages.length === 1 && !isLoading && (
            <div className="cyber-tools" aria-label="Guided cyber safety tools">
              <div className="cyber-tools-heading">
                <strong>What should we do?</strong>
                <span>Choose a guided path instead of starting with a blank chat.</span>
              </div>
              <div className="cyber-tools-grid">
                {tools.map((tool) => <CyberToolCard key={tool.label} tool={tool} onClick={() => handleTool(tool)} />)}
              </div>
            </div>
          )}

          {activeTool === "teach" && messages.length > 1 ? (
            <div className="mode-hint">🎓 Learning mode: ask for a lesson, scenario, flashcards or a quiz.</div>
          ) : null}
          {activeTool === "privacy" && messages.length > 1 ? (
            <div className="mode-hint">🔐 Safety mode: never paste passwords, OTPs, PINs, CVVs, recovery codes or tokens here.</div>
          ) : null}
          ...
          <form onSubmit={handleSend} className="chatbot-input-form">
            <textarea
              ...
              placeholder={location.pathname === "/check" ? "Ask about this suspicious message…" : "Message CyberRakshak AI…"}
              maxLength={2000}
              rows={1}
              disabled={isLoading}
              aria-label="Message CyberRakshak AI"
            />
      ...
      {!isOpen && (
        <button type="button" className="chatbot-toggle" onClick={toggleOpen} aria-label="Open CyberRakshak AI Safety Copilot" aria-expanded="false">
          <span className="toggle-icon">🛡️</span>
          <span className="toggle-label">Ask CyberRakshak</span>
