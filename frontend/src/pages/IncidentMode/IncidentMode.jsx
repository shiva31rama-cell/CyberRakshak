...
function IncidentMode() {
  ...
  const loadPlan = async (id) => {
    ...
    setError("");
    try {
      const r = await fetch(`${API_BASE}/incidents/${encodeURIComponent(id)}`);
      const d = await r.json();
      if (!r.ok || !d.success) throw new Error(d.message || "Unable to load the recovery plan");
      setPlan(d.plan);
    } catch (e) {
      setPlan(null);
      setError(e.message);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/incidents/scenarios`)
      ...
      .then((d) => { if (active && d.scenarios?.length) setScenarios(d.scenarios); })
      .catch(() => {});
    return () => { active = false; };
  }, []);
  ...
  const urgencyLabel = useMemo(() => !plan ? "" : plan.urgency === "critical" ? (telugu ? "ఇప్పుడే చర్య తీసుకోండి" : "Act now") : plan.urgency === "high" ? (telugu ? "త్వరగా చర్య తీసుకోండి" : "Act soon") : (telugu ? "జాగ్రత్తగా ఉండండి" : "Stay cautious"), [plan, telugu]);
  ...
  return <div className="incident-mode">
    ...
    {suggestedScenario && <div className="recovery-loading" role="status">{telugu ? "మీ చెక్ ఫలితానికి సరిపోయే రికవరీ మార్గాన్ని ఎంచుకున్నాం." : "We selected the recovery path that best matches your check result."}</div>}
