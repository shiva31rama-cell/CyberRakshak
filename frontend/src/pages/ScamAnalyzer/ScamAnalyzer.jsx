...
function RiskBadge({ result }) {
  ...
  return (
    <div className={`risk-badge risk-${result.color}`}>
      ...
      <span><strong>{result.label}</strong><small>{result.confidence} confidence</small></span>
    </div>
  );
}
...
function ScamAnalyzer() {
  ...
  const [ageGroup, setAgeGroup] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cyberrakshak:safety-profile"))?.ageGroup || "";
    } catch {
      return "";
    }
  });
  ...
  const analyze = async (event) => {
    ...
    try {
      ...
      const fallback = mode === "url" ? analyzeLocalUrl(value) : analyzeLocalText(value);
      if (fallback.success) {
        setResult(fallback);
        setUsingFallback(true);
        setError("The server is temporarily unavailable, so CyberRakshak switched to its local safety engine. No text was sent anywhere.");
      } else {
  ...
  return (
    <div className="analyzer-page">
      ...
      <section className="profile-strip" aria-label="Personalized safety profile">
        <div><strong>{t("personalized")}</strong><span>Only a self-declared age group and preferred language are used for guidance.</span></div>
        <div className="profile-controls">
          <select aria-label={t("ageGroup")} value={ageGroup} onChange={(event) => saveAgeGroup(event.target.value)}>
          ...
          <select aria-label={t("preferredLanguage")} value={language} onChange={(event) => syncLanguage(event.target.value)}>
      ...
      <section className="analyzer-shell">
        <div className="mode-tabs" role="tablist" aria-label="Analyzer input type">
          {[["message", "💬", t("message")], ["url", "🔗", t("url")], ["screenshot", "📸", t("screenshot")]].map(([value, icon, label]) => (
            <button key={value} className={mode === value ? "active" : ""} onClick={() => setMode(value)} type="button" role="tab" aria-selected={mode === value}>{icon} {label}</button>
          ))}
        </div>
        ...
        {mode === "screenshot" ? (
          <div className="screenshot-box">
            ...
            <button type="button" className="primary-action secondary-action" onClick={() => setMode("message")}>{t("analyzeSafely")}</button>
          </div>
        ) : (
          <form onSubmit={analyze} className="analyzer-form">
            <label htmlFor="analysis-input">{mode === "url" ? t("suspiciousUrl") : t("suspiciousMessage")}</label>
            {mode === "url" ? (
              <input id="analysis-input" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://example.com/..." autoComplete="off" inputMode="url" />
            ) : (
              <textarea id="analysis-input" value={text} onChange={(event) => setText(event.target.value)} placeholder="Paste the message exactly as you received it…" rows={8} />
            )}
            <div className="form-actions">
              <button id="run-analysis" type="submit" className="primary-action" disabled={loading}>{loading ? "Checking…" : `🔍 ${t("analyzeSafely")}`}</button>
              <button type="button" className="ghost-action" onClick={() => { setText(""); setUrl(""); setResult(null); setError(""); setUsingFallback(false); }}>{t("clear")}</button>
            </div>
          </form>
        )}
        ...
        {result ? (
          <section className="result-panel" aria-live="polite">
            <div className="result-heading">
              <div><span className="eyebrow">ANALYSIS RESULT</span><h2>{result.category === "unknown" ? "General safety assessment" : result.category.replaceAll("-", " ")}</h2></div>
              <RiskBadge result={result} />
            </div>
            {result.signals?.length ? (
              <div className="result-grid">
                <div className="result-card"><h3>🚩 Why it was flagged</h3><ul>{result.signals.map((signal) => <li key={signal.id}><strong>{signal.title}:</strong> {signal.reason}</li>)}</ul></div>
                <div className="result-card"><h3>✅ What to do</h3><ul>{result.recommendedActions?.map((action) => <li key={action}>{action}</li>)}</ul></div>
              </div>
            ) : (
            ...
            {result.nextStep ? <div className="next-step"><strong>Next step</strong><span>{result.nextStep}</span></div> : null}
      ...
      <section className="emergency-callout">
        <div><span className="eyebrow">{t("needHelpNow")}</span><h2>🆘 {t("emergencyCalloutTitle")}</h2><p>Open the incident-response checklist and official reporting guidance.</p></div>
        <a href="/emergency-help">{t("openEmergency")} →</a>
      </section>

      <section className="category-grid-section">
        <div className="section-heading"><span className="eyebrow">PROTECT YOURSELF</span><h2>Modern scam categories</h2><p>Focused scenarios instead of a generic trust score.</p></div>
        <div className="category-grid">{["💳 UPI & payment fraud", "🚨 Digital arrest", "💼 Fake job scams", "📱 SIM & telecom safety", "🎣 Phishing & smishing", "🎭 Impersonation", "📈 Investment scams", "💬 Social-media scams"].map((item) => <button type="button" key={item} className="category-card" onClick={() => { setMode("message"); setText(`${item}: please teach me the warning signs and safe next steps.`); window.scrollTo({ top: 0, behavior: "smooth" }); }}>{item}<span>→</span></button>)}</div>
      </section>

      <section className="coming-soon-section">
        <div className="section-heading"><span className="eyebrow">ROADMAP</span><h2>🚀 Coming soon</h2><p>These capabilities stay labelled until implementation and production checks are complete.</p></div>
        <div className="coming-grid">{comingSoon.map(([icon, title, description]) => <article key={title} className="coming-card"><span className="coming-icon">{icon}</span><div><h3>{title}</h3><p>{description}</p></div><span className="coming-badge">COMING SOON</span></article>)}</div>
      </section>
    </div>
  );
}
